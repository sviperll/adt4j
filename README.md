...
You can leave `valueClassName` parameter of `@GenerateValueClassForVisitor`-annotation out.
...
ADT4J allows to you to avoid manual extention of generated classes when recursive data-types are required.
To achieve this goal `selfReferenceVariableName` parameter is used.

```java
    @GenerateValueClassForVisitor(
        resultVariableName = "R",
        selfReferenceVariableName = "L",
        valueClassName = "List")
    interface OpenListVisitor<L, T, R> {
        R empty();
        R prepend(T head, L tail);
